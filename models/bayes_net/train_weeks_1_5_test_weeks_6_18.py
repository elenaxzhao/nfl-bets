...
def train_weeks_1_5_test_weeks_6_18():
    """Train on Weeks 1-5 of 2024 data and test on Weeks 6-18 of 2024 data"""
    print("NFL Bayesian Network - Train Weeks 1-5, Test Weeks 6-18")
    print("=" * 60)
    ...
    # Split into training (Weeks 1-5) and test (Weeks 6-18)
    training_data = full_2024_data[full_2024_data['week'] <= 5].copy()
    test_data = full_2024_data[full_2024_data['week'] >= 6].copy()
    
    print(f"Training data (Weeks 1-5): {len(training_data)} games")
    print(f"Test data (Weeks 6-18): {len(test_data)} games")
    
    # Train the model
    print("\n2. Training Bayesian Network on Weeks 1-5 of 2024...")
    bn.train(training_data)
    
    # Test the model on Weeks 6-18 of 2024 data
    print("\n3. Testing model on Weeks 6-18 of 2024...")
    evaluation = bn.evaluate_model(test_data)
    
    print(f"\n" + "="*60)
    print("RESULTS - WEEKS 1-5 TRAINED, WEEKS 6-18 TESTED")
    print("="*60)
    print(f"Training Data (Weeks 1-5, 2024): {len(training_data)} games")
    print(f"Test Data (Weeks 6-18, 2024): {len(test_data)} games")
    print(f"Accuracy on Weeks 6-18: {evaluation['accuracy']:.3f} ({evaluation['accuracy']*100:.1f}%)")
    print(f"Correct Predictions: {evaluation['correct_predictions']}/{evaluation['total_predictions']}")
    
    # Show some example predictions
    print(f"\n4. Example Weeks 6-18 predictions:")
    print("-" * 40)
    
    # Show first 5 test games (from Weeks 6-18)
    ...
    bn.save_model('nfl_bayesian_model_weeks_1_5_2024.pkl')
    print("Model saved as 'nfl_bayesian_model_weeks_1_5_2024.pkl'")
    ...
    print(f"\nTraining Data (Weeks 1-5) Home Win Rate: {training_home_win_rate:.3f} ({training_home_win_rate*100:.1f}%)")
    print(f"Test Data (Weeks 6-18) Home Win Rate: {home_win_rate:.3f} ({home_win_rate*100:.1f}%)")
    
    # Compare training vs test performance
    training_eval = bn.evaluate_model(training_data)
    print(f"\nTraining Accuracy (Weeks 1-5): {training_eval['accuracy']:.3f} ({training_eval['accuracy']*100:.1f}%)")
    print(f"Test Accuracy (Weeks 6-18): {evaluation['accuracy']:.3f} ({evaluation['accuracy']*100:.1f}%)")
    print(f"Overfitting Check: {training_eval['accuracy'] - evaluation['accuracy']:.3f} difference")
    
    return bn, evaluation

if __name__ == "__main__":
    model, results = train_weeks_1_5_test_weeks_6_18()
